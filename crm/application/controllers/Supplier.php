...
class Supplier extends MY_Controller {
	...
	public function __construct() {
		...
		$this -> membersdb = $this->Membersmodel;
                ...
                // 供货商品牌对应关系数据模型staff
                $this->load->model('CompanySellerBrandmodel');
                $this->companysellerbranddb = $this->CompanySellerBrandmodel;
	}
	...
	public function index() {
            ...
            if(!empty($searchtext)){//如果搜索内容不为空
                switch($searchtype){
                    ...
                    case 3://按照手机号搜索
                        ...
                        break;
                    case 4://按照所属卖家信息搜索
                        $maijiaxinxi = $this->membersdb->getMembersInfoBySearchFields(['realname'=>'like \'%'.$searchtext.'%\''],true);
            ...
            $data['offset'] = $offset;
            $result = $this -> membersdb -> MembersSupplierList($tablename,$data);
        ...
        */
        ...
        public function save(){
            if(IS_AJAX){
                ...
                if($result){
                    $supplierdata['brand']=  $this->input->post('brandname');
                    $supplierdata['m_seller']=  $this->input->post('sellerid'); // 卖家id
                    $supplierdata['m_buy']=  $result; // 供货商id
                    $this->staffmaster->trans_begin();
                    $supplierresult = $this->companysellerbranddb->saveSupplierBrandInfo($supplierdata);
                    if($supplierresult){
                        $this->staffmaster->trans_commit();
                        $this->master->trans_commit();
                        $result = array('info'=>1,'tip'=>$type ? '供货商更新成功！' : '添加供货商成功!');
                    }else{
                        $this->staffmaster->trans_rollback();
