...
class Page {
	 ...
	 */
	public function __construct($totalRows = '', $listRows = 20, $nowPage = 1, $parameter = array()) {
		$this -> p = 'page';
		//设置分页参数名称
		/* 基础设置 */
	...
	public function show() {
		...
		$str_parameter = $this->p . '=' . '[PAGE]';
		$count_parameter = count($this->parameter);
		if($count_parameter > 0){//有附加参数
			$parameter_str = array();
			foreach ($this->parameter as $k => $v) {
				$parameter_str[] = $k . '=' . $v;
			}
			$str_parameter = '?'.implode('&',$parameter_str).'&'.$str_parameter;
		}else{//没有附加参数
			$str_parameter = '?'.$str_parameter;
		}
		$page_ci = &get_instance();
		$page_contoller = $page_ci -> uri -> segment(1);
		$page_action = $page_ci -> uri -> segment(2);
		$page_url = $page_contoller . '/' . $page_action;
		$this -> url = site_url($page_url)  . $str_parameter;
		/* 计算分页信息 */
