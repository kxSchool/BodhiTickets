...
class Category extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->categorydb = $this->Categorymodel;

        $this->load->model('CategoryBrandmodel');
      //  $this->categorybranddb = $this->CategoryBrandmodel;
        $this->categorybranddb = new CategoryBrandmodel();
    }
    ...
    public function getBrand(){
        ...
        if($existflag){
            ...
            //获取品牌
            $brand = $this->categorydb->getAllBrandData();
            // 获取已选择品牌
            $selected = $this->categorydb->getCategorySelectedBrandDataByCategoryId($categoryid);
            foreach ($brand as $bkey=>$bval){
                if(in_array($bval['id'],$selected)){
                    $brand[$bkey]['selected'] = ' checked="checked" ';
                }else{
                    $brand[$bkey]['selected'] = '';
                }
            }
    ...
    public function setBrand(){
        ...
        if(empty($this->input->post('brandids'))){
        ...
        $data = array(
            ...
            'brand_ids' => trim($this->input->post('brandids'),',')
        );
        if($this->categorydb->saveCategoryAndBrandRelative($data,($this->input->post('relative')=='false') ? false : true )){
    ...
    public function category_edit(){
        ...
        if(!empty($CategoryInfo)){
            ...
            $this->load->view('category/categoryEdit',$CategoryInfo);
        }else{
            $dataTip['msg'] = '传递参数错误';
    ...
    public function categoryBrand(){
        ...
        //(3)通过分类连表查询关联的品牌
        $categoryBrands = $this->categorybranddb->getCategoryBrandInfo($cat_id);
        if(!empty($categoryBrands)){
            $data['brandsinfo'] = $categoryBrands;
        }

        $this->load->view('category/categoryBrand',$data);
    }
    ...
    public function updateSort()
    {
        if(IS_AJAX){
            ...
            $data = array();
            if(count($sort_arr) == count($brand_arr)){
                foreach($sort_arr as $key=>$val){
                    $data[] = array(
                        'cat_id' => $cat_id,
                        'brand_id' => $brand_arr[$key],
                        'sort' => $sort_arr[$key]
                    );
                }
            }
            ...
            if(!empty($data)){
                foreach($data as $v){
                    $updateResult = $this->categorybranddb->updateSort($v);
                    if(!$updateResult){
                        $returnData = array(
                            'info' => 0,
                            'tip' => '操作失败'
    ...
    public function categoryAttribute(){
        ...
        $this->load->view('category/categoryAttribute',$data);
    }
