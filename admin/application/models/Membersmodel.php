...
class Membersmodel extends CI_Model {
     ...
     * 会员列表
     */
    public function getSupplierList($tablename,$data){
        $tablename = substr($tablename,strlen($this->slave->dbprefix));;//去除表前缀
        ...
        if(!empty($data['sellerid'])){
            $where .= 'scsb.parentid '.$data['sellerid'].' AND ';
        }
        ...
        // 语句
        $searchSql = 'SELECT scsb.*,smb.* FROM '.$slavedb.'.'.$tablename.' smb,'
                .$slavedb.'.supplier_brand scsb '
                . 'WHERE scsb.shopid=smb.userid '.$where
                .$order.' '.$limit;
        ...
        // 查询所属卖家信息
        $supplierids = implode(',', array_column($result['rows'], 'parentid'));
