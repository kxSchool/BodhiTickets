 ...
 */
class TicketsCategorymodel extends CI_Model
{
    public $slave, $master;
     ...
     */
    public function saveCategoryAndShowRelative($data=[],$flag=false){
        if(empty($data['cat_id']) || empty($data['show_ids'])){
            return false;
        }
        if($flag){ //更新数据
            $this->master->where('cat_id',$data['cat_id']);
            if($this->master->update('category_show', $data)){
                return true;
            }else{
                return false;
            }
        }else{ //新增数据
            if($this->master->insert('category_show', $data)){
                return true;
            }else{
                return false;
            }
    ...
    public function getAllShowData(){
        $sql = 'SELECT `id`,`name` FROM `show` ORDER BY `id` ';
        $result = $this->slave->query($sql)->result_array();
        return empty($result) ? [] : $result;
    }
    /**
     ...
     * @return array 品牌id数据
     */
    public function getCategorySelectedShowDataByCategoryId($categoryid=0){
        if(empty($categoryid)){
            return [];
        }
        $sql = 'SELECT show_ids FROM category_show WHERE cat_id='.$categoryid;
        $result = $this->slave->query($sql)->result_array();
        if(empty($result[0]['show_ids'])){
            return [];
        }else{
            return explode(',', trim($result[0]['show_ids'],','));
        }
    }
