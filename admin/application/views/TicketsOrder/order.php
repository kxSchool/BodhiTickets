				...
				<div class="box">
					<div class="box-header with-border">
						<div class="pull-right">
							<form id="searchNewsForm" class="form-inline" method="post" action="<?php echo site_url('TicketsOrder/index');?>">
							...
							<table class="table table-bordered">
								...
								<tr>
									...
									<td>
										...
										<?php endif;?>
										<a class="btn btn-sm btn-info" href="<?php echo site_url('TicketsOrder/detailOrder');?>?order_id=<?php echo $v['order_id'];?>">查看订单</a>
									</td>
								</tr>
								<?php endforeach;?>
								<?php endif;?>
...
$(function(){
	...
	$("#exportExcel").click(function(){
		...
		var openUrl = '<?php echo site_url('TicketsOrder/exportOrderExcel')?>'+'?'+postStr;
...
function refundHandel(order_id){
	//首先弹出来退款申请的理由
	$.ajax({
		url : "<?php echo site_url('TicketsOrder/ajaxGetOrderInfo');?>",
		type : 'post',
		dataType : 'json',
		...
		success:function(data){
			if(data.info == 1){
				layer.confirm(data.data.refund_desc, {
					...
					$.ajax({
						url : "<?php echo site_url('TicketsOrder/ajaxRefundOrder');?>",
						type : 'post',
						dataType : 'json',
						data : {'order_id':order_id},
						success:function(data){
