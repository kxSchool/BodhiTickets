			...
			<div class="col-md-12">
				<div class="box">
					<div class="box-header with-border">
                        <?php if ($show_id){?>
						<a type="button" class="btn btn-success" href="<?php echo site_url('map/map_add');?>?show_id=<?php echo $show_id;?>"><span class="glyphicon glyphicon-plus"></span> 创建场馆座位图</a>
						<?php }?>
                        <div class="pull-right">
							<form id="searchNewsForm" class="form-inline" method="post" action="<?php echo site_url('map/manage');?>?show_id=<?php echo $show_id;?>">
								<div class="form-group">
									<select name="show_select" data-placeholder="选择场馆座位图" style="width:500px;" id="show_select" class="show_select" >
									</select>
					...
					<div class="box-body">
						<div class="tables">
							<table class="table table-bordered">
								...
								<tr>
									<td><?php echo $v['id'];?></td>
									<td align="center"><?php if(!empty($v['mini'])):;?><img src="<?php echo $v['mini'];?>" style="height:80px;vertical-align:middle;"><?php else:?>暂无Logo<?php endif;?></td>
									<td><?php echo $v['map_name'];?></td>
                                    <td><input type="checkbox" name="onoffswitchcategory" mapid="<?php echo $v['id'];?>" value="<?php echo $v['ismenu'];?>" <?php if($v['ismenu'] == 1):?>checked<?php endif;?> /></td>
									<td>
                                        <a class="btn btn-sm btn-info" href="<?php echo site_url('section/manage');?>?event_id=<?php echo $v['id'];?>&map_id=<?php echo $v['id'];?>">场馆区域</a>
										<a class="btn btn-sm btn-info" href="<?php echo site_url('map/map_edit');?>?id=<?php echo $v['id'];?>">编辑</a>
									</td>
								</tr>
								<?php endforeach;?>
								<?php endif;?>
...
<script>
    ...
    //表演关闭开启操作
		...
		$('input[name="onoffswitchcategory"]').on('switchChange.bootstrapSwitch', function(event, state) {
			...
			$.ajax({
				url : "<?php echo site_url('map/ismenuShow');?>",
				type : 'post',
				dataType : 'json',
				data : {'id':mapid,'ismenu':ismenuvalue},
				success:function(data){
					if(data.info != 1){
						layer.alert(data.tip, {icon: 5});
					}
