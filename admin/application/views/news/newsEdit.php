							...
							<form id="newsForm" class="form-horizontal" method="post" action="<?php echo site_url('news/saveNews');?>">
									...
									<div class="col-sm-10">
										<select class="form-control" name="catid" id="catid">
											<?php foreach($categorys as $v):?>
												...
												?>
												<option value="<?php echo $v['catid'];?>" <?php if(isset($v['childrenCount'])):?>disabled="disabled"<?php endif;?> <?php if($catid == $v['catid']):?>selected<?php endif;?>><?php echo $nbsp.$v['spacer'].$v['catname'];?></option>
											<?php endforeach;?>
										</select>
									</div>
								</div>
								<div class="form-group">
									...
									<div class="col-sm-10">
										...
										<input type="text" class="form-control" name="title" id="title" placeholder="文章名" value="<?php echo $title;?>" />
									</div>
								</div>
								<div class="form-group" id="ad_image_code">
									...
									<div class="col-sm-10">
										...
										<input id="thumb" type="hidden" name="thumb" value="<?php echo $thumb;?>" />
									</div>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
										<input type="text" class="form-control" name="keywords" id="keywords" placeholder="请输入文章关键词，用英文逗号分开" value="<?php echo $keywords;?>" />
									</div>
								</div>
								<div class="form-group">
									<label for="description" class="col-sm-2 control-label">文章描述：</label>
									<div class="col-sm-10">
										<textarea class="form-control" name="description" id="description" placeholder="输入文章描述"><?php echo $description;?></textarea>
									</div>
								</div>
								<div class="form-group">
									<label for="publishtime" class="col-sm-2 control-label">发布日期：</label>
									<div class="col-sm-10">
										<input type="text" class="form-control" name="publishtime" id="publishtime" placeholder="发布日期不填写，则为当前时间" value="<?php echo date('Y-m-d H:i:s',$publishtime);?>" />
									</div>
								</div>
								<div class="form-group">
									<label for="views" class="col-sm-2 control-label">访问量：</label>
									<div class="col-sm-10">
										<input type="text" class="form-control" name="views" id="views" placeholder="不填写默认为0" value="<?php echo $views;?>" />
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-2 control-label">外部链接：</label>
									<div class="col-sm-10">
										<input type="radio" name="islink" value="0" <?php if($islink == 0):?>checked<?php endif;?> /> 否
										<input type="radio" name="islink" value="1" <?php if($islink == 1):?>checked<?php endif;?> /> 是
									</div>
								</div>
								<div class="form-group" id="urlDiv" style='display:<?php if($islink == 0):?>none;<?php elseif($islink == 1):?>block;<?php endif;?>'>
									<label for="url" class="col-sm-2 control-label">外部链接URL：</label>
									<div class="col-sm-10">
										<input type="text" class="form-control" name="url" id="url" placeholder="请输入合法的URL" value="<?php echo $url;?>" />
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-2 control-label">文章状态：</label>
									<div class="col-sm-10">
										<input type="radio" name="status" value="1" <?php if($status == 1):?>checked<?php endif;?> /> 发布
										<input type="radio" name="status" value="2" <?php if($status == 2):?>checked<?php endif;?> /> 待发布
										<input type="radio" name="status" value="0" <?php if($status == 0):?>checked<?php endif;?>/> 草稿箱
									</div>
								</div>
	...
	<script>
		$(function(){
			...
			$("#thumb_input").fileinput({
				...
				<?php if(isset($show_thumb)):?>
				initialPreview: [
					'<img src="<?php echo $show_thumb;?>" class="file-preview-image">'
				],
				<?php endif;?>
				autoReplace: true,
				maxFileCount: 1,
