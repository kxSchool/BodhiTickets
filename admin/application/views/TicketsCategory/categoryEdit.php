					...
					<div class="box-body">
						<form id="cateForm" class="form-horizontal" method="post" action="<?php echo site_url('TicketsCategory/category_save');?>">
									...
									<select class="form-control" name="parentid" id="parentid">
										...
										<?php if(isset($showcategory)):?>
											<?php foreach($showcategory as $v):?>
												...
												?>
												//思路：当前id = 父类id
												<option value="<?php echo $v['cat_id'];?>" <?php if($cat_pid == $v['cat_id']):?>selected<?php endif;?>>
													<?php echo $nbsp;?><?php echo $v['spacer'].$v['cat_name'];?>
												</option>
											<?php endforeach;?>
										<?php endif;?>
									</select>
							...
							<div class="form-group">
								...
								<div class="col-sm-10">
									<input type="hidden" name="categoryid" value="<?php echo $cat_id;?>" />
									<input type="text" class="form-control" name="categoryname" id="categoryname" placeholder="输入分类名" value="<?php echo $cat_name;?>" />
								</div>
							</div>
							<div class="form-group">
								<label for="c" class="col-sm-2 control-label">权重(排序)：</label>
								<div class="col-sm-10">
									<textarea name="desc"  class="form-control" id="desc"  placeholder="请输入该分类的相关描述"><?php echo $cat_desc;?></textarea>
								</div>
							</div>
